...
/**
 * by:Soldier=  defaultTimeOutSeconds = 10;
 */
...
/**
 * by:Soldier  shouldUpdateNetworkActivityIndicator = YES
 */
...
- (void)addRequestHeader:(NSString *)header value:(NSString *)value
{
    ...
    [self.class setDict:requestHeaders value:value forKey:header];
}
...
{
    [self clearDelegatesAndCancelARC];
}


- (void)clearDelegatesAndCancelARC
{
    [[self cancelledLock] lock];
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!tempUserAgentString) {
            ...
            //[NSString stringWithFormat:@"%@/%@", [ASIHTTPRequest defaultUserAgentString],self.requestOnlyId];
		}
		if (tempUserAgentString) {
...
//			[self setPostBodyReadStream:[ASIInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
        ...
        // add by : Soldier
        if (!request) {
            [self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"request setReadStream  is nil",NSLocalizedDescriptionKey,nil]]];
            return;
        }
...
- (void)checkRequestStatus
{
    ...
    /**
     * by:Soldier  -1001 -1009 -1005  3次重试
     */
	if ([self shouldTimeOut] || self.responseStatusCode ==  NSURLErrorNetworkConnectionLost || self.responseStatusCode == NSURLErrorCannotFindHost) {
...
- (void)reportFinished
{
    ...
    NSDictionary *jsonDic = [self.responseString objectFromJSONString];
    if (jsonDic) {
        ...
        if (location > 0){
            ...
            self.responsejsonDict = [realJson objectFromJSONString];
            if (!self.responsejsonDict) {
                realJson = [realJson substringToIndex:realJson.length-1];
                NSUInteger location = [realJson rangeOfString:@"}" options:NSBackwardsSearch].length;
                if (location > 0) {
                    realJson = [realJson substringToIndex:location+1];
                    self.responsejsonDict = [realJson objectFromJSONString];
...
//        NSLog(@"Errorurl = %@ \r\n  Invalid-JSON 异常= :%@ ",[self url], self.responseString);
    ...
    /**
     * by:Soldier   clearDelegatesAndCancel  释放
     */
    [self clearDelegatesAndCancelARC];
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		if ([theRequest downloadDestinationPath]) {
			...
			[theRequest setRawResponseData:[NSMutableData dataWithData:[[self downloadCache] cachedResponseDataForURL:[theRequest url]]]];
		}
